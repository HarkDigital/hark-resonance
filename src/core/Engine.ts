...
export class Engine {
  ...
  constructor(
    ...
    this.renderer.setClearColor(0xeeebe4, 1)
    this.renderer.outputColorSpace = THREE.SRGBColorSpace
    // Khronos PBR Neutral: faithful product-shot colour on bright scenes (ACES would grey the whites)
    this.renderer.toneMapping = THREE.NeutralToneMapping
    this.renderer.toneMappingExposure = 1
    this.renderer.info.autoReset = false
    this.renderer.debug.checkShaderErrors = !import.meta.env.PROD

    this.studio = new Studio(this.renderer, this.scene, this.mobile)
    this.scene.add(this.studio.object)
