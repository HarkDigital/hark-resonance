...
export default function create(): Chapter {
  ...
  return {
    ...
    group,
    init(ctx) {
      if (1) ctx.stage.classList.add('is-dark')
      el('p', 'hud-eyebrow', 'Cymatics', ctx.stage).style.cssText = 'position:absolute;left:var(--gutter);top:var(--safe-top)'
      const h = el('h2', 'hud-h2', undefined, ctx.stage)
      h.innerHTML = 'Eleven ways to be <em>heard.</em>'
