...
export function createLoader(root: HTMLElement, { skip = false } = {}) {
  root.innerHTML = `<div style="position:absolute;inset:0;display:grid;place-items:center;background:var(--paper);font:500 12px var(--font-mono);letter-spacing:.14em;color:var(--ink);transition:opacity .6s">LOADING <span data-pct>000</span>%</div>`
